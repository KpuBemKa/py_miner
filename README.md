...
- Прописать команду `python -m venv venv`. Это создаст виртуальную среду, куда будут устанавливаться библиотеки (она нужна, чтобы эти библиотеки не конфликтовали с библиотеками в других проектах);
- Активировать виртуальную среду командой `.\venv\Scripts\activate`;
- Прописать команду `pip install -r .\requirements.txt`. Это установит все необходимые библиотеки в созданную виртуальную среду;
...
- Запустить скрипт:
    ...
    - Прописать следующую команду: `.\venv\Scripts\python.exe .\main.py <количество блоков в одной линии>`. Например `.\venv\Scripts\python.exe .\main.py 30`;
    - Это запустит скрипт, но он не начнет копать сразу;
...
У скрипта есть возможность корректировать количество блоков, через которое он будет пролетать. Это необходимо из-за того, что скорость персонажа непостоянна, и персонаж иногда пролетает на блок / пол блока больше, чем запрошено (не знаю почему; либо пинг, либо моджанг). В скрипт встроена возможность на один пролет удлинить количество пройденных блоков.
...
`STOP_KEY`: останавливает цикл клпания;
